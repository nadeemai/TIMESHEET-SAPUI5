...
_groupLeaveBalancesByEmployee: function(aLeaveBalances) {
  ...
  aLeaveBalances.forEach(function(leaveBalance) {
    ...
    oGroupedData[sEmployeeId].leaveTypes.push({
      ...
      usedLeave: parseFloat(leaveBalance.usedLeave || leaveBalance.usedLeaveDays || 0).toFixed(2),
        ...
        this._oUserDialog = new Dialog({
            ...
            content: [
                new SimpleForm({
                    ...
                    content: [
                        new Label({ text: "First Name" }),
                        new Input({ value: "{/userData/firstName}", required: true }),

                        new Label({ text: "Last Name" }),
                        new Input({ value: "{/userData/lastName}", required: true }),

                        new Label({ text: "Email" }),
                        new Input({ value: "{/userData/email}", type: "Email", required: true }),
                        ...
                        new Label({ text: "Manager" }),
new Select({
    selectedKey: "{/userData/managerId}",   // ✅ FIXED
    items: {
        path: "/availableManagers",        // list of managers
        template: new Item({
            key: "{ID}",            // ID from backend
            text: "{firstName} {lastName}" // Display label
        })
    },
  change: function (oEvent) {
        const data = oEvent.getSource().getSelectedItem().getBindingContext().getObject();
        oViewModel.setProperty("/userData/managerId", data.ID);
        oViewModel.setProperty("/userData/managerName", data.firstName + " " + data.lastName);
    }
}),
...
if (!oUserData.managerId && oUserData.managerName) {
    ...
    _createEmployeeInOData: function (oUserData, bRefresh = true) {
      ...
      var oEmployeeData = {
        ...
        managerID_ID: oUserData.managerId || null,
        managerName: oUserData.managerName || null,
        isActive: oUserData.status === "Active"
      };
...
sap.ui.define([
  ...
  return Controller.extend("admin.controller.Admin", {
    ...
    _loadOverallProgress: function () {
      ...
      oModel.read("/OverallProgressReport", {
        success: function (oData) {
          var overallData = oData.results || [];

          if (overallData.length > 0) {
            var totalBookedHours = overallData[0].totalBookedHours || 0;
            var totalAllocatedHours = overallData[0].totalAllocatedHours || 0;
            var totalRemainingHours = overallData[0].totalRemainingHours || 0;
            var averageUtilization = overallData[0].averageUtilization || 0;
            ...
            // Update projectHours data with the totalBookedHours
            var projectHours = oViewModel.getProperty("/projectHours") || [];
            if (projectHours.length > 0) {
              // Calculate total booked hours from all projects
              var calculatedTotalBookedHours = projectHours.reduce(function (sum, project) {
                return sum + (project.bookedHours || 0);
              }, 0);

              // Update the totalBookedHours with the value from the backend
              // or use the calculated value if backend doesn't provide it
              oViewModel.setProperty("/overallProgress/totalBookedHours",
                totalBookedHours > 0 ? totalBookedHours : calculatedTotalBookedHours);
            }
          }
        },
        error: function (oError) {
          console.error("Error loading overall progress data:", oError);
  ...
  // STEP 1: First get the Role GUID from backend
  this._getUserRoleIdByName(oUserData.role)
    .then(function (roleGuid) {

      // STEP 2: Read Employees to find backend GUID
      oModel.read("/Employees", {
        success: function (oData) {
          ...
          var oEmployeePayload = {
            ...
            lastName: oUserData.lastName,
            email: oUserData.email,
            userRole_ID: roleGuid,         // <-- correct GUID
            managerID_ID: oUserData.managerId || null,
            isActive: oUserData.status === "Active"
          };
          ...
          oModel.update(sPath, oEmployeePayload, {
            success: function (oData) {
              ...
              // var idx = aUsers.findIndex(u => u.userId === oUserData.userId);
              // if (idx !== -1) {
              //   aUsers[idx].role = oUserData.roleName;
              //   aUsers[idx].roleName = oUserData.roleName;
              //   aUsers[idx].userRole_ID = roleGuid;
              //   oViewModel.setProperty("/users", aUsers);
              // }
              resolve(oData)
    ...
    .catch(function (err) {
      console.error("Failed to load role ID:", err);
      MessageToast.show("Role not found, cannot update.");
    });
},
...
sap.ui.define([
  ...
  return Controller.extend("admin.controller.Admin", {
    ...
    onEmployeeChange: function (oEvent) {
      ...
      // 1️ get selected employee id
      let employeeId = oEvent.getParameter("selectedItem").getKey();
    ...
    _loadEmployees: function () {
      ...
      oModel.read("/Employees", {
        success: function (oData) {
          ...
          if (aFormattedUsers.length > 0) {
            let defaultEmployeeId = aFormattedUsers[0].userId;
            oViewModel.setProperty("/selectedEmployee", defaultEmployeeId);
    ...
    _updateEmployeeInOData: function (oUserData, bRefresh = true) {
      ...
      oModel.read("/Employees", {
        success: function (oData) {
          ...
          // Comprehensive payload for update - INCLUDING ALL FIELDS
          var oEmployeePayload = {
            firstName: oUserData.firstName,
            lastName: oUserData.lastName,
            email: oUserData.email,
            roleName: oUserData.role, // Ensure role is saved as roleName in backend
            managerID_ID: oUserData.managerId || null,
            isActive: oUserData.status === "Active"
          };
...
sap.ui.define([
  ...
  "sap/m/VBox",
  "sap/m/ComboBox"
], function (Controller, MessageToast, JSONModel, Fragment, MessageBox, Filter, FilterOperator, Select, Item, Input, DatePicker, Button, Dialog, Text, Label, SimpleForm, ToolbarSpacer, Toolbar, ODataModel, VBox, ComboBox) {
  "use strict";
  ...
  return Controller.extend("admin.controller.Admin", {
    ...
    _loadAdminTimesheetData: function (employeeId, weekStart, weekEnd) {
      ...
      oModel.read("/Timesheets", {
        success: function (oData) {
          ...
          let employeeEntries = allResults.filter(item =>
            item.employeeEmpID === employeeId
          );

          function normalizeDate(d) {
            return new Date(d).toISOString().split("T")[0];
          }
          // 2️⃣ Filter by week (workDate between weekStart & weekEnd)
          // 2️⃣ Filter by backend weekStartDate & weekEndDate
          let weekEntries = employeeEntries.filter(item => {
            let backendStart = normalizeDate(item.weekStartDate);
            let backendEnd = normalizeDate(item.weekEndDate);

            let frontStart = normalizeDate(weekStart);
            let frontEnd = normalizeDate(weekEnd);

            return backendStart === frontStart && backendEnd === frontEnd;
          });

          // 3️⃣ Format table structure
          let formatted = that._formatAdminTimesheet(weekEntries);
    ...
    // Update the onDatePickerChange function in Admin.controller.js:
    ...
    onDatePickerChange: function (oEvent) {
      ...
      // Calculate Monday (start of week)
      let day = selectedDate.getDay(); // Sun = 0
      let monday = new Date(selectedDate);
      monday.setDate(selectedDate.getDate() - (day === 0 ? 6 : day - 1));

      // Save weekStart in model
      oModel.setProperty("/currentWeekStart", monday);

      // Update week days label UI
      this._updateWeekDays(monday);

      // Compute week end
      let weekEnd = this._getWeekEnd(monday);

      // Load time entries for the selected employee
      let employeeId = oModel.getProperty("/selectedEmployee");
      if (employeeId) {
        this._loadAdminTimesheetData(employeeId, monday, weekEnd);
      }

      sap.ui.core.BusyIndicator.hide();
    },

    _updateWeekDays: function () {
      let oModel = this.getView().getModel();
      let start = new Date(oModel.getProperty("/currentWeekStart"));
        ...
        success: function (oData) {
          ...
          Promise.all(deletePromises).then(function () {
            ...
            Promise.all(createPromises).then(function () {
              MessageToast.show("Timesheet recorded successfully for employee: " + employeeId);
              // Reload timesheet data to reflect changes
              that._loadTimesheetData(employeeId);
            }).catch(function (error) {
              console.error("Error creating timesheet entries:", error);
              MessageToast.show("Error saving some timesheet entries");
            });
      ...
      oModel.read("/Employees", {
        success: function (oData) {
          ...
          if (aFormattedUsers.length > 0) {
            ...
            // Load timesheet for this employee immediately
            that._loadAdminTimesheetData(defaultEmployeeId);
          }
  ...
  return Controller.extend("admin.controller.Admin", {
    ...
    onTaskDetailPress: function (oEvent) {
      try {
        ...
        // Create a dialog with a custom style class
        var oDialog = new sap.m.Dialog({
          title: "Week Task Details",
        ...
        // Add custom CSS styles to match the image
        var sCustomCSS = `
      .tsDayCard {
        margin-bottom: 10px;
        padding: 10px;
    ...
    onEmployeeChange: function (oEvent) {
      ...
      // 2️store selected employee in model
      oViewModel.setProperty("/selectedEmployee", employeeId);
      let weekStart = oViewModel.getProperty("/currentWeekStart");
    ...
    onDatePickerChange: function (oEvent) {
      ...
      // 1️⃣ Compute Monday (start of week)
      let day = selectedDate.getDay(); // Sun = 0
      let monday = new Date(selectedDate);
      monday.setDate(selectedDate.getDate() - (day === 0 ? 6 : day - 1));

      // 2️⃣ Save weekStart in model (clean format)
      oModel.setProperty("/currentWeekStart", monday);

      // 3️⃣ Validate week start (important!)
      let currentWeekStart = new Date(oModel.getProperty("/currentWeekStart"));
      if (isNaN(currentWeekStart)) {
        console.error("Invalid week start:", oModel.getProperty("/currentWeekStart"));
        sap.ui.core.BusyIndicator.hide();
      ...
      // 5️⃣ Update week days label UI
      this._updateWeekDays(weekStart);

      // 6️⃣ Load time entries for the selected employee
      let employeeId = oModel.getProperty("/selectedEmployee");
      if (employeeId) {
        this._loadAdminTimesheetData(employeeId, weekStart, weekEnd);
      }
...
sap.ui.define([
  ...
  return Controller.extend("admin.controller.Admin", {
    ...
    onHoursChange: function (oEvent) {
      ...
      var sProperty = oSource.getBindingPath("value");

      // Get the entry and update the hours
      var iIndex = parseInt(sPath.split("/")[2]);
      var entry = timesheetEntries[iIndex];

      // Update the day hours
      entry[sProperty] = oEvent.getParameter("value");
      ...
      // Update the model
      oModel.setProperty("/timesheetEntries", timesheetEntries);
    },
