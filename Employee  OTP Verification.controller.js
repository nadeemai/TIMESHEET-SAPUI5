    ...
    return Controller.extend("employee.controller.OTPVerification", {
        ...
        _onRouteMatched: function (oEvent) {
            ...
            if (!encryptedId || encryptedId.trim() === "") {
                // Get dynamic encrypted ID
                this._getDynamicEncryptedId().then(encryptedId => {
                    this._linkToken = encryptedId;
                    
                    // Update URL with dynamic encrypted ID
                    this._oRouter.navTo("otp-verification-with-token", {
                        encryptedId: encryptedId
                    }, true);
                    
                    // Continue with the flow
                    this._continueWithToken();
                }).catch(err => {
                    console.error("Failed to get dynamic encrypted ID:", err);
                    MessageBox.error("Failed to generate verification token. Please try again.");
                });
            } else {
                this._linkToken = encryptedId;
                this._continueWithToken();
        ...
        _getLinkToken: function() {
            ...
            if (this._linkToken) {
                return Promise.resolve(this._linkToken);
            }
            ...
            // Get dynamic encrypted ID as fallback
            return this._getDynamicEncryptedId().then(encryptedId => {
                this._linkToken = encryptedId;
                oModel.setProperty("/linkToken", this._linkToken);
                oModel.setProperty("/encryptedId", this._linkToken);
        ...
        _getDynamicEncryptedId: function() {
                ...
                this._getCurrentUser().then(user => {
                    if (user && user.employeeID) {
                        ...
                        if (storedId) {
                            ...
                            reject(new Error("No employee ID available for encryption"));
                        }
                    }
                }).catch(err => {
        ...
        _getCurrentUser: function() {
            return new Promise((resolve, reject) => {
                // Try to get user from the user API service
        ...
        async _startAuthenticationFlow() {
            try {
                const linkToken = await this._getLinkToken();
        ...
        onManualGenerateOTP: function () {
            this._getLinkToken().then(linkToken => {
                if (!linkToken || linkToken.trim() === "") {
        ...
        onAfterRendering: function() {
            ...
            if (!this._linkToken) {
                this._getLinkToken().then(linkToken => {
                    ...
                    console.error("Failed to get link token after rendering:", err);
                });
            }
        }
    });
