...
sap.ui.define([
  ...
  return Controller.extend("manager.controller.Manager", {
    ...
    onInit: function () {
      ...
      // Restore selected employee from localStorage if exists
    ...
    _loadEmployees: function () {
      ...
      oModel.read("/MyTeam", {
        success: function (oData) {
          ...
          // Check if there's a stored employee ID and select it
          let storedEmployeeId = localStorage.getItem("selectedEmployeeId");
          if (storedEmployeeId) {
            // Find the employee in the loaded list
            let storedEmployee = filtered.find(emp => emp.userId === storedEmployeeId);
            if (storedEmployee) {
              // Set the selected employee in the model
              oVM.setProperty("/selectedEmployee", storedEmployeeId);
              oVM.setProperty("/selectedEmployeeName", `${storedEmployee.firstName} ${storedEmployee.lastName}`);
              
              // Load the timesheet data for this employee
              let weekStart = oVM.getProperty("/currentWeekStart");
              let weekEnd = that._getWeekEnd(weekStart);
              that._loadAdminTimesheetData(storedEmployeeId, weekStart, weekEnd);
              
              // Set the selected item in the list after a delay to ensure the list is rendered
              setTimeout(function() {
                let oList = that.byId("employeeList");
                if (oList) {
                  let aItems = oList.getItems();
                  for (let i = 0; i < aItems.length; i++) {
                    let oItem = aItems[i];
                    let oContext = oItem.getBindingContext();
                    if (oContext && oContext.getProperty("userId") === storedEmployeeId) {
  ...
  return Controller.extend("manager.controller.Manager", {
    ...
    onInit: function () {
      ...
      let oVM = new JSONModel({
        ...
        projectsData: [], // Add projects data array
         assignedProjects: []
      });
      this.getView().setModel(oVM);
    ...
    this._loadAssignedProjects();
    },

    // Add these functions to your controller

// Function to open the assign project dialog
onAssignProject: function() {
    // Create a dialog if it doesn't exist yet
    if (!this._oAssignProjectDialog) {
        this._oAssignProjectDialog = sap.ui.xmlfragment(
...
_loadProjectsForAssignment: function(oDialogModel) {
    ...
    oModel.read("/MyProjects", {
        success: (oData) => {
            ...
            // Ensure each project has proper id and name properties
            projects = projects.map(p => ({
                id: p.projectID,
                name: p.projectName || p.name || "Unnamed Project"
            }));
...
_loadEmployeesForAssignment: function(oDialogModel) {
    ...
    oModel.read("/MyTeam", {
        success: (oData) => {
            BusyIndicator.hide();
            let employees = oData.results || [];
            
            // Ensure each employee has proper userId and name properties
            employees = employees.map(e => ({
                employeeID: e.employeeID,
                assignedBy: e.managerName,
                fullName: `${e.firstName} ${e.lastName}`,
                isActive: e.isActive
            }));
            
            oDialogModel.setProperty("/employees", employees);
            console.log("Employees loaded:", employees); // Debug log
        },
        error: (oError) => {
            BusyIndicator.hide();
...
onAssignProjectConfirm: function() {
    ...
    // Call the function to actually assign the project
    this._assignProjectToEmployee(oProject, oEmployee);
},
...
_assignProjectToEmployee: function(oProject, oEmployee) {
    ...
    oModel.create("/assignProjectToEmployee", oPayload, {
        success: function(oData) {
            ...
            resolve(oData)
            // Refresh the assigned projects list
            that._loadAssignedProjects();
        },
        error: function(oError) {
            BusyIndicator.hide();
            MessageBox.error("Failed to assign project: " + (oError.message || "Unknown error"));
        }
    });
},

// Function to load assigned projects
_loadAssignedProjects: function() {
    let oModel = this.getOwnerComponent().getModel("timesheetServiceV2");
    let oViewModel = this.getView().getModel();
    let that = this;
    ...
    oModel.read("/AssignedProjects", {
        success: function(oData) {
            BusyIndicator.hide();
            let assignments = oData.results || [];
            oViewModel.setProperty("/assignedProjects", assignments);
        },
        error: function() {
            BusyIndicator.hide();
            MessageToast.show("Error loading assigned projects.");
            oViewModel.setProperty("/assignedProjects", []);
        }
    });
},
...
onRemoveProjectAssignment: function(oEvent) {
    ...
    MessageBox.confirm(
        "Are you sure you want to remove this project assignment?",
        {
            onClose: function(sAction) {
                if (sAction === MessageBox.Action.OK) {
                    this._removeProjectAssignment(oAssignment);
...
formatAssignmentStatusState: function(sStatus) {
    ...
    // Function to update status with rejection reason
    _updateTimesheetStatusWithReason: function (entryId, status, reason) {
      let oModel = this.getOwnerComponent().getModel("timesheetServiceV2");
      let oVM = this.getView().getModel();
      let that = this;
      ...
      // Update the status in the backend with rejection reason
      oModel.update("/TeamTimesheets(" + entryId + ")", {
        status: status,
        ...
        success: function () {
          BusyIndicator.hide();
          MessageToast.show("Timesheet status updated to " + status);
    ...
    _loadEmployees: function () {
      ...
      let oVM = this.getView().getModel();
      BusyIndicator.show(0);
      // Changed from /AllEmployees to /MyTeam
      oModel.read("/MyTeam", {
        success: function (oData) {
          let list = oData.value || oData.results || [];
          // Normalize employees
    ...
    _loadProjectsData: function () {
      ...
      oModel.read("/MyProjects", {
        success: (oData) => {
          ...
          let projectsData = oData.results || [];
          oViewModel.setProperty("/projectsData", projectsData);

          // Also populate the projects array for selects
...
onAssignProjectConfirm: function() {
    ...
    let oProject = aProjects.find(p => p.id === sProjectId);
    let oEmployee = aEmployees.find(e => e.userId === sEmployeeId);
...
_assignProjectToEmployee: function(oProject, oEmployee) {
    ...
    let oPayload = {
        projectID: oProject.id,
        projectName: oProject.name || oProject.projectName,
        employeeID: oEmployee.userId,
        employeeName: `${oEmployee.firstName} ${oEmployee.lastName}`,
    };
